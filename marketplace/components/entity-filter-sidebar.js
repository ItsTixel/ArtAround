  ...
  _render() {
    ...
    this.innerHTML = `
      <aside class="${GLASS} p-6 sticky top-24 max-h-[calc(100vh-120px)] overflow-y-auto overflow-x-hidden max-[900px]:static max-[900px]:max-h-none text-slate-800 dark:text-slate-100 font-sans">
        <!-- Header del foglio filtri: visibile solo in modalità "bottom sheet"
             (mobile ≤900px, vedi visits.css). Su desktop è display:none. -->
        <div class="sheet-head">
          <span class="sheet-grab" aria-hidden="true"></span>
          <span class="sheet-title">Filtri</span>
          <button type="button" class="sheet-x" data-sheet-close aria-label="Chiudi i filtri">&times;</button>
        </div>
        ...
        <div class="group pt-6 pb-6 border-t border-slate-400/20 first:pt-0 first:border-t-0">
          ...
          <label class="search relative block">
            ...
            <input id="author-q" type="search" placeholder="Cerca un autore…" autocomplete="off" value="${this._esc(s.authorQuery)}" class="w-full pl-6 pr-2 py-2.5 border-0 border-b border-slate-400/20 bg-transparent text-sm text-slate-800 dark:text-slate-100 placeholder:italic placeholder:text-slate-500 dark:placeholder:text-slate-400 outline-none focus:border-slate-800 dark:focus:border-white transition-colors">
          </label>
